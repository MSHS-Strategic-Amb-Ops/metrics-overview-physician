...
dept <-  tbl(conn2, "CLARITY_DEP") %>%
  head(10) %>%
  collect()
# order_conn <- tbl(conn5, dbplyr::ident_q('"CABOODLE"."ProcedureOrderFact"'))
# order_proc <- orders_raw_tbl %>%
#   filter(DESCRIPTION %LIKE% "%ECHO%") %>%
#   head(1000) %>%
#   collect()
# 
# order_fact <- order_conn %>%
#   head(1000) %>%
#   collect()
# 
...
#   collect()
# 
# department <- access_raw_tbl %>%
#   filter(DEPARTMENT_NAME == "55 E 34TH ST RAD BONE DENSITY") %>%
#   collect()
# 
# 
# 
...
access <- access_raw_tbl %>%
  ...
  filter(SCHED_METHOD %LIKE% "%Fast Pass%") %>%
  collect()

fatspass <- access_raw_tbl %>%
  filter(PAT_ID == "Z1947030") %>%
  collect()
  


f_sched_appt <- f_sched_appt_tbl %>%
  filter(PAT_ENC_CSN_ID == "100181537803") %>%
  collect()
...
no_show <- access_raw_tbl %>%
  ...
  mutate(appt_status = ifelse(DERIVED_STATUS_DESC == "Canceled" & (cancel_lead_days <=0 | is.na(cancel_lead_days)), "SameDay_Canceled", DERIVED_STATUS_DESC)) %>%
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PRC_NAME, PRC_ID, appt_status) %>%
  summarise(total = n()) %>%
  collect() %>%
